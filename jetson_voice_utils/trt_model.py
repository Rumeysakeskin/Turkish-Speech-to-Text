...
from jetson_voice_utils.trt_builder import build_engine, TRT_LOGGER
from jetson_voice_utils.trt_binding import Binding
...
class TRTModel:
    ...
    def __init__(self, config, dynamic_shapes=None, *args, **kwargs):
        ...
        if self.trt_engine is None:
            raise IOError(f'failed to load TensorRT engine from {self.config["model_path"]}')
    ...
    def execute(self, inputs, sync=True, return_dict=False, **kwargs):
        ...
        print("TensorRT in jetson voice utils !!!!!")
        ...
        # copy outputs to CPU
        for output in self.outputs:
            output.host = np.zeros(output.shape, dtype=trt.nptype(output.dtype))
